...
const SUPABASE_URL = "https://kkmgvmdjdijbovmhzwul.supababase.co";
    ...
    >
      {partner.revenue_active && (
        <div style={{
          ...
          background: "#4ae04a22",
          borderBottom: "1px solid #4ae04a44",
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "#555", marginTop: 2, fontFamily: "'DM Mono',monospace" }}>
            {partner.market}  ·  {partner.corridor}
          </div>
        </div>
      ...
      <div style={{ borderTop: "1px solid #1a1a1a", paddingTop: 16 }}>
        {[
          ...
          <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            <span style={{
              fontSize: 11,
              color: k === "REVENUE SHARE" && partner.revenue_active ? "#4ae04a" : "#888",
              fontFamily: "'DM Mono',monospace", textAlign: "right", maxWidth: "60%"
            }}>{v}</span>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${SUPABASE_URL}/rest/v1/partners?select=*&order=id`, {
      ...
      .then(res => {
        if (!res.ok) throw new Error(`HTTP ${res.status} — check your SUPABASE_URL and SUPABASE_KEY`);
        return res.json();
      })
      .then(data => {
  ...
  if (error) return (
    <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center", padding: 40 }}>
      <div style={{ maxWidth: 600, width: "100%" }}>
        ...
        <div style={{ color: "#f55", fontFamily: "'DM Mono',monospace", fontSize: 13, marginBottom: 16 }}>Connection error</div>
        <div style={{ color: "#666", fontFamily: "'DM Mono',monospace", fontSize: 11, background: "#0e0e0e", padding: 16, borderRadius: 8, border: "1px solid #1e1e1e" }}>{error}</div>
        <div style={{ color: "#444", fontSize: 12, marginTop: 16, lineHeight: 1.6 }}>
          Check that SUPABASE_URL and SUPABASE_KEY are set correctly in App.jsx, and that RLS is disabled on the partners table.
        </div>
      </div>
    </div>
  );
          ...
          <div style={{ display: "flex", gap: 2, marginBottom: 24, borderRadius: 8, overflow: "hidden", height: 6 }}>
            {STAGES.map(s => {
              ...
              return (
                <div key={s.id}
                  title={`${s.label}: ${count}`}
                  style={{ flex: count / partners.length * 100, background: COLORS[s.id].accent, minWidth: count > 0 ? 2 : 0 }}
                />
              );
            })}
          </div>
